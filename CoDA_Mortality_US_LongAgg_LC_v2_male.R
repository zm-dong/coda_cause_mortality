...
# where there are 12 causes, so nrow(datiM[1,,]) is the number of years for cause 1
...
# ================================================================
# life table deaths for females and males
# ================================================================
...
dx.com.M <- cbind(datiM.rela1[,,1],datiM.rela1[,,2],datiM.rela1[,,3],datiM.rela1[,,4]
                  ,datiM.rela1[,,5],datiM.rela1[,,6],datiM.rela1[,,7],datiM.rela1[,,8]
                  ,datiM.rela1[,,9],datiM.rela1[,,10]
                  ,datiM.rela1[,,11],datiF.rela1[,,12])#,datiF.rela1[,,13],datiF.rela1[,,14],datiF.rela1[,,15],datiF.rela1[,,16])

#zeroes are replaced in this data:
dx.com.M_0 <- cbind(datiM.rela1_0[,,1],datiM.rela1_0[,,2],datiM.rela1_0[,,3],datiM.rela1_0[,,4]
                    ,datiM.rela1_0[,,5],datiM.rela1_0[,,6],datiM.rela1_0[,,7],datiM.rela1_0[,,8]
                    ,datiM.rela1_0[,,9],datiM.rela1_0[,,10]
                    ,datiM.rela1_0[,,11],datiM.rela1_0[,,12]) 
...
dataM.colm1 <- c(dataM.colm[,,1],dataM.colm[,,2],dataM.colm[,,3],dataM.colm[,,4],dataM.colm[,,5],dataM.colm[,,6],dataM.colm[,,7],dataM.colm[,,8]
                 ,dataM.colm[,,9],dataM.colm[,,10]
                 ,dataM.colm[,,11],dataM.colm[,,12])

# weight plot array dimensions should be 1, number of age bands (9), number of causes (12)
weight.plot <- array(dataM.colm1,dim=c(1,9,12))

dea.rela.dist <-  dati.M.mean/sum(dati.M.mean)
...
# for US mortality by cause, use all years

# training data: years 1979 to 2012 (US: years 1979 to 2001)
...
dx <- as.matrix(dx.com.M)
#dx <- as.matrix(dx.com.M_0)
dx <- dx / rowSums(dx)
...
# Fit the models
# ================================================================
# forecasting total life table deaths by age and gender
M.dx.nat3_fore = matrix(NA, 9, ih)
for (j in 1:9)
{
  for(ik in 1:ih)
  {
    M.dx.nat3_fore[j,ik] = exp(forecast(auto.arima(log(M.dx.nat3[j,1:ik])), h = 1)$mean)
  }
}

# forecast life table deaths (datiF.rela1) and then divide the result by exposure to get mortality rate
colnames(M.dx.nat3_fore) <- as.character(years.for) #all causes, by age and year
rownames(M.dx.nat3_fore) <- as.character(ages)
...
model.fit.for.lc <- COD.LC(mx=m.M.cause.i, t=ih, k=length(CoD),ages=ages,years=years)
...
# The ILR was applied to centred data, rather than the original composition
...
dx.ar <- array(dx.com.M, dim=c(length(years.fit.1),m,length(CoD)),dimnames=list(as.character(years.fit.1[1]:(years.fit.1[length(years.fit.1)])),as.character(ages),CoD))
dx.ar1 <- array(acomp(dx.com.M), dim=c(length(years.for),m,length(CoD)),dimnames=list(as.character(years.for[1]:(years.for[length(years.for)])),as.character(ages),CoD))
...
#constructing interval forecast
  interval_forecast_clr_full <- CLR_fun_int(object = data, ncomp_method = "fixed", ncomp_tuning = 0.001, fh = ih,
  ...
  # create plot clr
  interval_CLR_m <- ggplot(data = NULL, aes(x = year, y = proportion, group = CoD, colour = CoD)) + 
...
data <- dx.cent
#constructing interval forecast
interval_forecast_ilr_full <- ILR_fun_int(object = data, ncomp_method = "fixed", ncomp_tuning = 0.001, fh = ih,
                                      fore_method = "rwf", B = 399, K = 1, sig = 20)
...
# create plot ILR
interval_ILR_m <- ggplot(data = NULL, aes(x = year, y = proportion, group = CoD, colour = CoD)) + 
...
alpha <- 0.7 #optimal alpha for males is 0.7 (cardio)
helmert <- TRUE 

#while (alpha < 1){
  alpha.M <- alfa(data, alpha, helmert)$aff
  rownames(alpha.M) <- as.character(years.fit.1)
  # Apply SVD to the alpha transformed data
  par <- svd(alpha.M, nu=3, nv=3)
  U <- par$u 
  V <- par$v
  S <- diag(par$d)
...
clr_proj_m <- ggplot(data = NULL, aes(x = year, y = proportion, group = CoD, colour = CoD)) + 
  ...
  #scale_x_continuous("Year", breaks = seq(1979, 2051, 4)) +
  ggtitle("CLR") +
  #ggtitle("CLR (zeros replaced by 0.5 count)") +
  theme(plot.title = element_text(face = "bold",
                                  margin = margin(10, 0, 10, 0),
                                  size = 14)) +
  theme(legend.position = "none") +
...
ilr_proj_m <- ggplot(data = NULL, aes(x = year, y = proportion, group = CoD, colour = CoD)) + 
  ...
  #scale_x_continuous("Year", breaks = seq(1979, 2051, 4)) +
  ggtitle("ILR") + 
  #ggtitle("ILR (zeros replaced by 0.5 count)") + 
  theme(plot.title = element_text(face = "bold",
                                  margin = margin(10, 0, 10, 0),
                                  size = 14)) +
  theme(legend.position = "none") +
...
#lee-carter plot
lc_proj_m <- ggplot(data = NULL, aes(x = year, y = proportion, group = CoD, colour = CoD)) + 
...
#savepdf("us_lc_alpha_proj_m", width = 35, height = 12, toplines = 0.8)
lc_proj_m + alpha_proj_m +
plot_layout(ncol = 3)
#dev.off()

# Group plots together
  #savepdf("us_all_proj_m_0.7_100yrs", width = 33, height = 13, toplines = 0.8)
  ...
  savepdf("us_clr_zeros_m", width = 44, height = 13, toplines = 0.8)
  par(mar=c(2, 2, 2, 2), xpd=TRUE)
  clr_proj_m + clr_proj_m_0.25 + clr_proj_m_0.5 + 
    plot_layout(ncol = 4)
  dev.off()
  
  # zero replacements: group plots together for ILR
  savepdf("us_ilr_zeros_m", width = 44, height = 13, toplines = 0.8)
  par(mar=c(2, 2, 2, 2), xpd=TRUE)
  ilr_proj_m + ilr_proj_m_0.25 + ilr_proj_m_0.5 + 
    plot_layout(ncol = 4)
  dev.off()
  ...
  #constructing interval forecast
  interval_forecast_full <- alpha_fun_int(data, a = 0.7, ncomp_method = "fixed", ncomp_tuning = 0.001, fh = ih,
                                          fore_method = "rwf", B = 399, K = 1, sig = 20)
...
# plot interval forecasts
  ...
  # create plot alpha
  interval_alpha_m <- ggplot(data = NULL, aes(x = year, y = proportion, group = CoD, colour = CoD)) + 
    ...
    #savepdf("us_interval_m_0.7", width = 32, height = 12, toplines = 0.8)
    par(mar=c(2, 2, 2, 2), xpd=TRUE)
    interval_CLR_m + interval_ILR_m + interval_alpha_m
    plot_layout(ncol = 3)
    #dev.off()
