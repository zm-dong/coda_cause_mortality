...
# michelle's notes: this alpha is different from the alpha transformation! How did he pick 0.2?
...
alpha_fun_int <- function(object, a, ncomp_method, ncomp_tuning = 0.001, fh,
                          fore_method = c("ets", "arima", "rwf"), B = 399, K = 1, sig)
  
{
  object_alpha = alfa(object, a)$aff #alpha used in alpha transformation, creates the alpha-transformed matrix of the data in the real space
  rownames(object_alpha) <- as.character(years.fit.1)
  n_year = nrow(object_alpha) #number of years = number of rows (ok)
  n_age = ncol(object_alpha) #number of ages - number of columns (not ok but only the label, this should be age bands * causes)

  # ================================================================
  # michelle's edits from here (adding in forecasting using LC CODA)
  
  # SVD for the alpha-transformed matrix (ok this is the same as the forecast used in the original script, but we need to specify nu = 3 and nv = 3)
    #SVD_decomp = svd(object_alpha) 
  SVD_decomp <- svd(object_alpha, nu=3, nv=3) #adding in the other factors for LC forecasting
  U <- SVD_decomp$u 
  V <- SVD_decomp$v
  S <- diag(SVD_decomp$d)
  ...
  for(iw in 1:fh)
  {
    for(ij in 1:(B * K))
    {
      d_x_t_star_fore[iw,ij,] = alfainv_mod(t(pred_resize[,ij,iw]), a) #alfainv because we applied the alpha transformation earlier, but take the alfainv_mod version
    }
  }
  rm(iw); rm(ij)
