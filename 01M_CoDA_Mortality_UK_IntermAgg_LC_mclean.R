...
packages <- c("demography", "stats", "strucchange", "tables", "calibrate", "compositions", "abind", "MLmetrics", "tseries", "vars", "rTensor", "r.jive", "FactoMineR", "Metrics", "rainbow", "shape", "ggplot2", "reshape2", "viridis", "patchwork")
...
dummy <- 0.5 # to remove the zeroes
...
dea.F.total <- datiF.rela1[,,1] + datiF.rela1[,,2] +datiF.rela1[,,3] +datiF.rela1[,,4]+datiF.rela1[,,5]+datiF.rela1[,,6]+datiF.rela1[,,7]+datiF.rela1[,,8]
+datiF.rela1[,,9]+datiF.rela1[,,10]
+datiF.rela1[,,11]

dea.M.total <- datiM.rela1[,,1] + datiM.rela1[,,2] +datiM.rela1[,,3]+datiM.rela1[,,4]+datiM.rela1[,,5]+datiM.rela1[,,6]+datiM.rela1[,,7]+datiM.rela1[,,8]
+datiM.rela1[,,9]+datiM.rela1[,,10]
+datiM.rela1[,,11]
...
weight.plot <- array(dataM.colm1,dim=c(1,9,11))
...
alpha <- 0.1
  ...
  #alpha <- 0.1 #optimal alpha for males is 0.5 (cardio)
  #alpha <- 0.9 #optimal alpha for males (resp)
  #alpha <- 0.7 #run for results tables
...
#while (alpha < 1){
  ...
  pro.dea.clr.LC <- t(apply(clr.proj.for.one.ar, MARGIN=c(1,3),FUN=sum)) #this one should be the same as CT CODA
  pro.dea.ilr.LC <- t(apply(ilr.proj.for.one.ar, MARGIN=c(1,3),FUN=sum)) #this one should be the same as CT CODA
  ...
  #in sample RMSE
  print(alpha)
  print(rmse_clr_t*100)
  print(mae_clr_t*100)
  print(rmse_ilr_t*100)
  print(mae_ilr_t*100)
  print(rmse_alpha_t*100)
  print(mae_alpha_t*100)
  #alpha <- alpha + 0.1
  #}
