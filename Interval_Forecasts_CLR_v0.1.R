...
CLR_fun_int <- function(object, ncomp_method, ncomp_tuning = 0.001, fh,
                          fore_method = c("ets", "arima", "rwf"), B = 399, K = 1, sig)
  
{
  object_CLR = clr(object) #alpha used in alpha transformation, creates the alpha-transformed matrix of the data in the real space
  rownames(object_CLR) <- as.character(years.fit.1)
  n_year = nrow(object_CLR) #number of years = number of rows (ok)
  n_age = ncol(object_CLR) #number of ages - number of columns (not ok but only the label, this should be age bands * causes)
  ...
  SVD_decomp <- svd(object_CLR, nu=3, nv=3) #adding in the other factors for LC forecasting
  U <- SVD_decomp$u 
  V <- SVD_decomp$v
  S <- diag(SVD_decomp$d)
  ...
  resi = as.matrix(t(object_CLR - fit))
  ...
  # transform back
  d_x_t_star_fore = array(NA, dim = c(fh, B * K, (n_age)))
  for(iw in 1:fh)
  {
    for(ij in 1:(B * K))
    {
      d_x_t_star_fore[iw,ij,] = clrInv(t(pred_resize[,ij,iw])) #clrInv because we applied the CLR transformation earlier
    }
  }
  rm(iw); rm(ij)
